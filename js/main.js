function 검색(select_type,input_searche_value){
    ...
    // const input_value =document.getElementById('input_searche_value').value ///////////////////////////////////////////////////////////////////////////////////////
    ...
    // 검색 fetch 요청 보내고 목록받기
    fetch(`http://localhost:5000/s?condition=${select_type}&input=${input_searche_value}`)
...
function add(link_url, link_img, name, brand, type, ingredient){
    ...
    // 저장 fetch 요청 보내기 ----------------------
    fetch(`http://localhost:5000/add?userid=${user_id}&link_url=${link_url}&link_img=${link_img}&name=${name}&brand=${brand}&type=${type}&ingredient=${ingredient}`)
...
function del(userid,name){
    ...
    // 항목삭제 fetch 전송
    fetch(`http://localhost:5000/del?userid=${userid}&name=${name}`) 
    .catch((error) =>
        document.querySelector("#container").innerHTML += `
        <tr>
            <td colspan="4" class='del'>Error occurred.</td>
...
function load(){
    ...
    // userid 입력 받기
    const p = document.getElementById('input_userid').value;

    // 불러오기 fetch 전송
    fetch(`http://localhost:5000/load/${p}`)
    .then((response)=>response.json())
...
function crawlingadd(url, img, name, brand, type, ingredient){ ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // 저장 fetch 요청 보내기 ----------------------
    fetch(`http://localhost:5000/crawlingadd?url=${url}&img=${img}&name=${name}&brand=${brand}&type=${type}&ingredient=${ingredient}`)
...
function 크롤링(url,type){ ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // 불러오기 fetch 전송
    fetch(`http://localhost:5000/crawling?url=${url}&type=${type}`)
    .then((response)=>response.json())
    .then((elem) => {
        document.querySelector("#container tbody").innerHTML += `
